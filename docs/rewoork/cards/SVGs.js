...
export function getSvg(card){
    ...
    switch(card.type){
        case C.CARD_MOVE   : return moveSvg(teamClass, card);
        case C.CARD_ATTACK : return attackSvg(teamClass, card);
        case C.CARD_LEAP   : return leapSvg(teamClass, card);
        case C.CARD_LEAP   : return leapSvg(teamClass, card);
        case C.CARD_SPLIT  : return splitSvg(teamClass, card);
        case C.CARD_SWITCH : return switchSvg(teamClass, card);
...
return `
    <svg class="card ${teamClass}" id="card${card.id}">
        ...
        <text class="cardLabel" dominant-baseline="text-bottom" text-anchor="middle" x="50%" y="110px">${card.type.toUpperCase()}</text>
    </svg>
`;
}


function attackSvg(teamClass, card) {

    return `
        <svg class="card ${teamClass}" id="card${card.id}">
            <!-- ATTACK : can GO anywhere-->
            <!-- their area -->
            <polygon class="adv" points="-10,-10 -10,50 90,5 90,-10"/>
            <!-- my area -->
            <polygon class="mine" points="-10,130 -10,50 90,5 90,130"/>
            ...
            <!--move-->
            <line stroke="black" stroke-width="1" x1="15" x2="50" y1="25" y2="90"/>
            ...
            <!-- dest -->
            <circle class="active mine" cx="15" cy="25" r="5" stroke="black" stroke-width="2"/>
            <!-- origin -->
            <circle class="active mine" cx="50" cy="90" r="8" stroke="black" stroke-width="3"/>
...
function switchSvg(teamClass, card) {
    return `
        <svg class="card ${teamClass}" id="card${card.id}">
            <!-- ATTACK : can GO anywhere-->
            ...
            <!-- their area -->
            <polygon class="adv" points="-10,-10 -10,50 90,64 90,-10"/>
            <!-- my area -->
            <polygon class="mine" points="-10,130 -10,50 90,64 90,130"/>
            
            
            <!-- their border-->
            <line class="adv" stroke-width="3" x1="-10" x2="90" y1="50" y2="64"/>
            <!-- my border-->
            <line class="mine" stroke-dasharray="15 15" stroke-width="3" x1="-10" x2="90" y1="50" y2="64"/>
            ...
            <!-- dest -->
            <circle class="active mine" cx="15" cy="25" r="8" stroke="black" stroke-width="3"/>
            <!-- origin -->
            <circle class="active adv" cx="50" cy="90" r="8" stroke="black" stroke-width="3"/>
